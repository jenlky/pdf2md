...
exports.extractNumHeader = (string) => {
  const nestedNum = string.split(',')[0]
  const splitNum = nestedNum.split('.')

  const numHeader = []
  for (let x = 1; x < splitNum.length; x++) {
    numHeader.push(splitNum[x])
  }

  const firstNum = splitNum[0].split(' ')
  numHeader.unshift(firstNum[firstNum.length - 1])
  return numHeader
}
